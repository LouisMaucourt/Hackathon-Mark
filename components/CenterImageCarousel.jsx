...
const CenterImageCarousel = ({ slides, autoSlideInterval = 5000 }) => {
                ...
                >
                    {slides.map((slide, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`relative overflow-hidden transition-all duration-500 flex justify-center ${isCenter ? 'scale-100' : 'scale-75'}`}
                                style={{
                                    width: `${100 / slidesToShow}%`,
                                    transition: 'transform 0.5s ease-in-out'
                                }}
                            >
                                <div className="p-2 w-full h-[75vh] flex flex-col gap-9">
                                    <img
                                        src={slide.src}
                                        alt={slide.alt}
                                    ...
                                    <div className={`z-10 flex flex-col items-center justify-center transition-all ${isCenter ? 'opacity-100 ' : 'opacity-0'}`}>
                                        <h4 className='text-center'>{slide.titre}</h4>
